...
sum_bus_2 = df_bus_2["Distance"].sum()
sum_bus_2A = df_bus_2A["Distance"].sum()
sum_bus_2B = df_bus_2B["Distance"].sum()
sum_bus_3 = df_bus_3["Distance"].sum()
sum_bus_3A = df_bus_3A["Distance"].sum()
sum_bus_4 = df_bus_4["Distance"].sum()
sum_bus_5 = df_bus_5["Distance"].sum()
sum_bus_6 = df_bus_6["Distance"].sum()
sum_bus_8 = df_bus_8["Distance"].sum()
sum_bus_9 = df_bus_9["Distance"].sum()
sum_bus_11X = df_bus_11X["Distance"].sum()
sum_bus_13X = df_bus_13X["Distance"].sum()
sum_bus_U6 = df_bus_U6["Distance"].sum()
sum_bus_500 = df_bus_500["Distance"].sum()

sum_array = [sum_bus_2, sum_bus_2A, sum_bus_2B, sum_bus_3, sum_bus_3A, sum_bus_4, sum_bus_5, sum_bus_6, sum_bus_8, sum_bus_9, sum_bus_11X, sum_bus_13X, sum_bus_U6, sum_bus_500]

for x in sum_array:
    round(x, 1)

bus_array = ['2', '2A', '2B', '3', '3A', '4', '5', '6', '8', '9', '11X', '13X', 'U6', '500']

distance_data = {'Bus': bus_array, 'Distance': sum_array}
distance_df = pd.DataFrame(distance_data, columns = ['Bus','Distance'])

distance_df.plot.bar(x='Bus', y='Distance', rot=0, color='#ff007f')
plt.ylabel('Distance in miles')
plt.title('Distance travelled by each bus daily')
plt.show()
